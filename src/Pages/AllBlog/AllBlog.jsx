...
const AllBlog = () => {
  ...
  const getAllBlog = async () => {
    const res = await axios.get(`/allBlog?category=${category}&title=${search}`);
    return res;
  };
  ...
  return (
    ...
    <div className="p-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 container mx-auto gap-5 ">
        { isLoading? <p>loading....</p> : blogs?.data?.map((blog) => (
          <AllBlogCard key={blog?._id} blog={blog} setCategory={setCategory}></AllBlogCard>
        ))}
      </div>
    </div>
  );
